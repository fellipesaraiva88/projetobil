...
export default function App() {
  // --- STATE ---
  // Changed default tab to 'simulator'
  const [activeTab, setActiveTab] = useState<'simulator' | 'projects' | 'dashboard' | 'assistant'>('simulator');
  ...
  const renderSimulator = () => (
    <div className="pb-24 animate-in fade-in duration-500">
      <header className="mb-6 text-center">
        <h1 className="text-3xl font-bold text-slate-900">Simulador do Bill</h1>
        <p className="text-slate-500">Tire uma foto e veja como a pintura vai ficar na hora.</p>
      </header>
      <ImageSimulator />
      
      <div className="mt-8 p-6 bg-blue-50 rounded-xl border border-blue-100 text-center">
        <h3 className="font-bold text-blue-900 mb-2">Gostou do resultado?</h3>
        <p className="text-blue-700 mb-4">Crie uma nova obra agora mesmo para salvar este orçamento.</p>
        <button 
          onClick={() => { setActiveTab('projects'); setIsProjectModalOpen(true); }}
          className="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold shadow-md hover:bg-blue-700 transition-colors"
        >
          Criar Nova Obra
        </button>
      </div>
    ...
    return (
      <div className="space-y-6 pb-24">
        <header className="mb-6">
          ...
          <p className="text-slate-500">Resumo do seu negócio</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-gradient-to-br from-slate-700 to-slate-800 text-white border-none">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-slate-300 text-sm">Obras Ativas</p>
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40 pb-safe">
        <div className="flex justify-around items-center h-16">
          <button 
            onClick={() => { setActiveTab('simulator'); setDetailView(null); }}
            className={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'simulator' ? 'text-secondary' : 'text-slate-400'}`}
          >
            <Wand2 size={24} />
            <span className="text-xs mt-1 font-bold">Simular</span>
          </button>
          <button 
